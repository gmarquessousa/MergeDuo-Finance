    ...
    TimeProvider clock) : ICopilotFinanceService
{
        ...
        IReadOnlyList<PurchaseSimulationInstallmentResponse> installments)
    {
        ...
        var reason = paymentType == "cash"
            ? "Compra à vista analisada no mês da compra e nos 3 meses seguintes."
            : "Compra parcelada analisada durante todos os meses com parcelas e mais 3 meses após a última parcela.";
        ...
        PurchaseSimulationOverallRiskMetricsResponse overallRiskMetrics)
    {
        ...
        {
            "A compra não cria transação real, é apenas simulação."
        };
        ...
        if (overallRiskMetrics.ProjectedLowestBalance < 0)
        {
            ...
            attentionPoints.Add("Existe saldo diário projetado abaixo de zero na janela analisada.");
        }
        ...
        {
            riskFacts.Add("A compra aumenta a quantidade de dias abaixo da margem de segurança.");
            attentionPoints.Add("A margem de segurança fica mais pressionada após a compra.");
        }

        if (overallRiskMetrics.MonthsWithNegativeBalance.Count > 0)
        {
            riskFacts.Add($"Há saldo projetado abaixo de zero em {string.Join(", ", overallRiskMetrics.MonthsWithNegativeBalance)}.");
        }
        ...
        {
            positiveFacts.Add($"O pior mês da janela termina com saldo projetado de {Money(overallRiskMetrics.WorstProjectedEndBalance)}.");
        }

        if (overallRiskMetrics.AdditionalNegativeBalanceDays == 0)
        {
            positiveFacts.Add("A compra não cria dias adicionais com saldo negativo.");
        }

        if (overallRiskMetrics.RiskSignals.Contains(SignalNoCriticalRiskDetected, StringComparer.Ordinal))
        {
            positiveFacts.Add("Não foram detectados sinais críticos de risco na janela analisada.");
        }

        if (overallRiskMetrics.WorstProjectedEndBalance < overallRiskMetrics.SafetyMarginAmount)
        {
            attentionPoints.Add($"O pior mês termina abaixo da margem de segurança de {Money(overallRiskMetrics.SafetyMarginAmount)}.");
        }
        ...
        var safetyMarginSummary = overallRiskMetrics.ProjectedDaysBelowSafetyMargin > overallRiskMetrics.BaselineDaysBelowSafetyMargin
            ? $"A quantidade de dias abaixo da margem de segurança de {Money(overallRiskMetrics.SafetyMarginAmount)} aumenta de {overallRiskMetrics.BaselineDaysBelowSafetyMargin} para {overallRiskMetrics.ProjectedDaysBelowSafetyMargin} dias."
            : $"A quantidade de dias abaixo da margem de segurança de {Money(overallRiskMetrics.SafetyMarginAmount)} permanece em {overallRiskMetrics.ProjectedDaysBelowSafetyMargin} dias.";
        ...
        return new PurchaseSimulationAiAnalysisDataResponse(
            ...
            $"A análise considera {YearMonth.FromDate(analysisWindow.From).Value} até {YearMonth.FromDate(analysisWindow.To).Value}.",
        ...
        PurchaseSimulationPaymentScheduleAnalysisResponse paymentScheduleAnalysis)
    {
        if (paymentType == "credit_card")
        {
            return $"Compra \"{description}\" de {Money(amount)} no cartão em {paymentScheduleAnalysis.TotalInstallments} parcela(s), realizada em {purchaseDate:yyyy-MM-dd}.";
        }

        return $"Compra \"{description}\" de {Money(amount)} à vista em {purchaseDate:yyyy-MM-dd}.";
    }
    ...
    private static string SimulationAiContextText(
        ...
        PurchaseSimulationOverallRiskMetricsResponse overallRiskMetrics)
    {
        var payment = paymentType == "credit_card" ? "cartão de crédito" : "pagamento à vista";
        return string.Join(" ", new[]
        {
            $"Simulação de compra: {description}, {Money(amount)}, {payment} em {purchaseDate:yyyy-MM-dd}.",
            $"Janela analisada: {YearMonth.FromDate(analysisWindow.From).Value} até {YearMonth.FromDate(analysisWindow.To).Value}.",
            ...
            "A compra não persiste transação.",
            "Use estes dados para avaliar se a compra pode comprometer a organização financeira."
        });
    }
    ...
    private static string MonthSummaryText(CopilotMonthSummaryResponse response)
    {
        ...
        return $"Resumo de {response.Period.YearMonth}{mergeText}: entradas {Money(response.Totals.Entradas)}, saídas {Money(response.Totals.Saidas)}, saldo final {Money(response.Totals.Saldo)}, patrimônio {Money(response.Totals.Patrimonio)}, cartão de crédito {Money(cardTotal)} e {response.ProjectedCount} movimentos projetados.";
    }
    ...
    {
        var mergeText = response.Scope == "merged" ? " considerando o merge ativo" : "";
        return $"Levantamento de {response.From:yyyy-MM-dd} a {response.To:yyyy-MM-dd}{mergeText}: entradas previstas {Money(response.Totals.Entradas)}, saídas previstas {Money(response.Totals.Saidas)}, aportes {Money(response.Totals.Aportes)} e patrimônio projetado ao fim do período de {Money(response.Totals.Patrimonio)}.";
    }
    ...
    private static string SimulationText(CopilotPurchaseSimulationResponse response)
    {
        var payment = response.PaymentType == "credit_card"
            ? $"{response.Installments.Count}x no cartão"
            : "à vista";
        return $"Simulação da compra \"{response.Description}\" de {Money(response.Amount)} {payment}: janela {YearMonth.FromDate(response.AnalysisWindow.From).Value} até {YearMonth.FromDate(response.AnalysisWindow.To).Value}, menor saldo projetado {Money(response.OverallRiskMetrics.ProjectedLowestBalance)} em {response.OverallRiskMetrics.ProjectedLowestBalanceDate:yyyy-MM-dd}, pior mês {response.OverallRiskMetrics.WorstMonth} com saldo final projetado de {Money(response.OverallRiskMetrics.WorstProjectedEndBalance)}. Nenhuma transação foi criada.";
    }
    ...
    private static string CardsText(CopilotCardsResponse response)
    {
        ...
        return response.Cards.Count == 0
            ? $"Nenhum cartão ativo encontrado{mergeText}."
            : $"{response.Cards.Count} cartão(ões) ativo(s) encontrado(s){mergeText}.";
    }
